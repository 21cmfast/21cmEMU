...
class DefaultEmulatorV1(nn.Module):
    ...
    def __init__(self, negative_slope: float = 0.1):
        ...
        self.ps_conv_0 = nn.ConvTranspose2d(1000, 256, kernel_size=(4, 2), stride=1, padding=0)  # valid
        self.ps_conv_1 = nn.ConvTranspose2d(256, 256, kernel_size=(7, 3), stride=1, padding=0)   # same (crop)
        self.ps_conv_2 = nn.ConvTranspose2d(256, 256, kernel_size=(3, 3), stride=1, padding=0)   # valid
        self.ps_conv_3 = nn.ConvTranspose2d(256, 128, kernel_size=(7, 3), stride=1, padding=0)   # same (crop)
        self.ps_conv_4 = nn.ConvTranspose2d(128, 128, kernel_size=(7, 3), stride=1, padding=0)   # valid
        self.ps_conv_5 = nn.ConvTranspose2d(128, 64, kernel_size=(3, 1), stride=1, padding=0)    # valid
        self.ps_conv_6 = nn.ConvTranspose2d(64, 64, kernel_size=(5, 3), stride=1, padding=0)     # same (crop)
        self.ps_conv_7 = nn.ConvTranspose2d(64, 32, kernel_size=(7, 3), stride=1, padding=0)     # same (crop)
        self.ps_conv_8 = nn.ConvTranspose2d(32, 32, kernel_size=(7, 3), stride=1, padding=0)     # same (crop)
        self.ps_conv_10 = nn.ConvTranspose2d(32, 8, kernel_size=(3, 1), stride=1, padding=0)     # valid
        self.ps_conv_11 = nn.ConvTranspose2d(8, 8, kernel_size=(9, 3), stride=1, padding=0)      # same (crop)
        self.ps_conv_13 = nn.ConvTranspose2d(8, 8, kernel_size=(9, 3), stride=1, padding=0)      # same (crop)
        self.ps_conv_14 = nn.ConvTranspose2d(8, 1, kernel_size=(11, 3), stride=1, padding=0)     # same (crop)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ps = ps.contiguous().view(-1, 720)  # Make contiguous after cropping, then flatten
...
def convert_dense_weights(tf_weights: List[np.ndarray]) -> Tuple[torch.Tensor, torch.Tensor]:
...
def convert_conv_transpose_weights(tf_weights: List[np.ndarray]) -> Tuple[torch.Tensor, torch.Tensor]:
...
def transfer_weights(pt_model: DefaultEmulatorV1, tf_weights: Dict[str, List[np.ndarray]]):
    ...
    # Tb head
    tb_tf_names = ['Dense_Tb_0', 'Dense_Tb_1', 'Dense_Tb_2', 'Dense_Tb_3', 
                   'Dense_Tb_4', 'Dense_Tb_5', 'Dense_Tb_6', 'Dense_Tb_final']
    tb_pt_names = ['tb_0', 'tb_1', 'tb_2', 'tb_3', 'tb_4', 'tb_5', 'tb_6', 'tb_final']
    for tf_name, pt_name in zip(tb_tf_names, tb_pt_names):
        w, b = convert_dense_weights(tf_weights[tf_name])
        getattr(pt_model, pt_name).weight.data = w
        getattr(pt_model, pt_name).bias.data = b
    
    # xHI head
    xhi_tf_names = ['Dense_xHI_0', 'Dense_xHI_1', 'Dense_xHI_2', 'Dense_xHI_3', 
                    'Dense_xHI_4', 'Dense_xHI_final']
    xhi_pt_names = ['xhi_0', 'xhi_1', 'xhi_2', 'xhi_3', 'xhi_4', 'xhi_final']
    for tf_name, pt_name in zip(xhi_tf_names, xhi_pt_names):
        w, b = convert_dense_weights(tf_weights[tf_name])
        getattr(pt_model, pt_name).weight.data = w
        getattr(pt_model, pt_name).bias.data = b
    
    # Ts head
    ts_tf_names = ['Dense_Ts_0', 'Dense_Ts_1', 'Dense_Ts_2', 'Dense_Ts_3', 
                   'Dense_Ts_4', 'Dense_Ts_final']
    ts_pt_names = ['ts_0', 'ts_1', 'ts_2', 'ts_3', 'ts_4', 'ts_final']
    for tf_name, pt_name in zip(ts_tf_names, ts_pt_names):
        w, b = convert_dense_weights(tf_weights[tf_name])
        getattr(pt_model, pt_name).weight.data = w
        getattr(pt_model, pt_name).bias.data = b
    
    # discont head
    dc_tf_names = ['Dense_discont_0', 'Dense_discont_1', 'Dense_discont_2', 
                   'Dense_discont_3', 'Dense_discont_4', 'Dense_discont_final']
    dc_pt_names = ['discont_0', 'discont_1', 'discont_2', 'discont_3', 'discont_4', 'discont_final']
    for tf_name, pt_name in zip(dc_tf_names, dc_pt_names):
        w, b = convert_dense_weights(tf_weights[tf_name])
        getattr(pt_model, pt_name).weight.data = w
        getattr(pt_model, pt_name).bias.data = b
    ...
    for tf_name, pt_name in zip(tau_tf_names, tau_pt_names):
        w, b = convert_dense_weights(tf_weights[tf_name])
        getattr(pt_model, pt_name).weight.data = w
        getattr(pt_model, pt_name).bias.data = b
    
    # UVLF head
    uvlf_tf_names = ['Dense_UVLF_0', 'Dense_UVLF_1', 'Dense_UVLF_2', 
                     'Dense_UVLF_3', 'Dense_UVLF_4', 'Dense_UVLF_final']
    uvlf_pt_names = ['uvlf_0', 'uvlf_1', 'uvlf_2', 'uvlf_3', 'uvlf_4', 'uvlf_final']
    for tf_name, pt_name in zip(uvlf_tf_names, uvlf_pt_names):
        w, b = convert_dense_weights(tf_weights[tf_name])
        getattr(pt_model, pt_name).weight.data = w
        getattr(pt_model, pt_name).bias.data = b
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', default=os.path.expanduser(
        "~/.local/share/py21cmEMU/21cmEMU/21cmEMU"),
        help="Path to TensorFlow SavedModel")
    parser.add_argument('--output', default='default_model.pt',
