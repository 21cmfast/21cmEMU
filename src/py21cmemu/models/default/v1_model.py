...
class DefaultEmulatorV1(nn.Module):
    ...
    def __init__(self, negative_slope: float = 0.3):
        ...
        self.ps_decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                256, 256, kernel_size=(3, 3), stride=(1, 2), output_padding=(0, 1)
            ),
            nn.LeakyReLU(negative_slope),
            ...
            nn.ConvTranspose2d(
                128, 128, kernel_size=(3, 3), stride=(2, 1), output_padding=(1, 0)
            ),
            nn.LeakyReLU(negative_slope),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat(
            [tb_out, xhi_out, ts_out, discont_out, ps_out, tau_out, uvlf_out], dim=-1
        )
